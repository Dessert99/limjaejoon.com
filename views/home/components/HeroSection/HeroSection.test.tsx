...
describe('HeroSection', () => {
  it('headline 을 페이지 유일의 h1 으로 렌더한다', () => {
    ...
    expect(headings[0]).toHaveTextContent('DESIGN ENGINEER');
  });
  ...
  it('heading 으로 이름 붙은 region 랜드마크다', () => {
    ...
    expect(
      screen.getByRole('region', { name: 'DESIGN ENGINEER' })
    ).toBeInTheDocument();
  });
  ...
  it('마퀴 반복 복사본은 h1 하나만 남기고 접근성 트리에서 숨긴다', () => {
    ...
    const copies = screen.getAllByText('DESIGN ENGINEER');
  ...
  it('인물 사진을 alt 와 함께 보여준다', () => {
    ...
    expect(
      screen.getByAltText('유럽 구도심 골목에 선 임재준')
    ).toBeInTheDocument();
  });
});
