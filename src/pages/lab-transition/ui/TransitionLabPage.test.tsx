...
describe('TransitionLabPage', () => {
  it('컨트롤·에디터·프리뷰·코드 패널을 모두 조립한다', () => {
    ...
    expect(
      screen.getByRole('heading', { level: 1, name: 'transition' })
    ).toBeInTheDocument();
    expect(
      screen.getByLabelText('cubic-bezier 곡선 에디터')
    ).toBeInTheDocument();
  ...
  it('프리셋을 바꾸면 코드 패널에 즉시 반영된다', () => {
    ...
    expect(
      screen.getByText('transition: transform 600ms linear 0ms;')
    ).toBeInTheDocument();
  });
  ...
  it('프로퍼티를 바꾸면 코드 패널의 대상 프로퍼티가 바뀐다', () => {
    ...
    expect(
      screen.getByText('transition: opacity 600ms ease 0ms;')
    ).toBeInTheDocument();
  });
});
