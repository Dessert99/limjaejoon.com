...
- `.css.ts`는 그 컴포넌트 전용이므로 컴포넌트 폴더 안에 `{Name}.css.ts`로 함께 둔다 (폴더 구조는 @docs/conventions/folder-structure.md §2·§3 참조). 컴포넌트는 같은 폴더의 스타일을 `import * as s from './{Name}.css'`로 참조
