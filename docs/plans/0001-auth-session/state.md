...
- [x] 구현 (Phase 1-7 모두 ✅)
- [x] 코드 검토 (code-reviewer + security-auditor + accessibility-tester — 1라운드 완료, Critical/Major 모두 반영)
- [x] 회고 (`reports/2026-04-30-auth-session.md`)
...
Phase 7 완료(2026-04-30): 프론트 테스트 14개 파일 / 68 테스트 통과 (Vitest + Testing Library + MSW).

- `tests/lib/auth/safeReturnTo.test.ts` (16 tests): open redirect 화이트리스트·protocol-relative·백슬래시·xss 스킴·루프 prefix 차단
- `tests/lib/auth/verifySession.test.ts` (6): 쿠키 없음 즉시 redirect / 200 user 반환 / 401·5xx redirect / Cookie 헤더 검증 / returnTo 인코딩
- `tests/lib/api/client.test.ts` (5, MSW 사용): 사전 refresh 분기 / 만료 임박 시 refresh 후 원 요청 / 동시 요청 mutex 1회 / 401 fallback 재시도 / refresh 실패 시 reject
- `tests/features/auth/api/{signup,login,logout,refresh,getMe}.test.ts` (19): apiClient 호출 인자 + tokenStore setAccessExpiresAt/clearAuth 부수효과 검증
- `tests/features/auth/hooks/{useSignup,useLogin,useLogout,useMe}.test.tsx` (12): TanStack mutation/query + me 캐시 갱신(setQueryData) / logout queryClient.clear()
- `tests/features/auth/components/LoginForm.test.tsx` (7, RHF + a11y): label-input 연결 / 잘못된 이메일 aria-invalid+role=alert / 짧은 비밀번호 / 정상 submit mutate 호출 / 401 root 에러 매핑
- 인프라 변경: `vitest.config.ts`에 `@/` 별칭 + `@vanilla-extract/vite-plugin` 추가. devDep `@vanilla-extract/vite-plugin`.

이전: Phase 6 완료(2026-04-30): 라우트 페이지 4개 — login/signup/(protected) layout/me.
...
이전: Phase 1 완료(2026-04-26) — backend deps, env.validation(Joi), data-source, app.module, 루트 .env 단일화(POSTGRES\_\*), docker-compose volumes에 .env 마운트, app.controller/service 제거, 차단 훅에서 .env 블록 제거.
...
cookie-parser + ValidationPipe + CORS + Swagger. 사용자 검증 액션: `docker compose up -d --build backend && docker compose exec backend npm run migration:run -w backend && open http://localhost:4000/api/docs`로 signup/login/refresh/me 수동 호출.
...
- `frontend/vitest.config.ts` — `@/` 별칭 + `@vanilla-extract/vite-plugin` 추가
- `frontend/package.json` — devDep `@vanilla-extract/vite-plugin`
- `frontend/tests/lib/auth/safeReturnTo.test.ts` (16 tests)
- `frontend/tests/lib/auth/verifySession.test.ts` (6 tests, vi.mock cookies/redirect/fetch)
- `frontend/tests/lib/api/client.test.ts` (5 tests, MSW로 인터셉터 통합 검증)
- `frontend/tests/features/auth/api/{signup,login,logout,refresh,getMe}.test.ts` (19 tests)
- `frontend/tests/features/auth/hooks/{useSignup,useLogin,useLogout,useMe}.test.tsx` (12 tests, QueryClientProvider wrapper)
- `frontend/tests/features/auth/components/LoginForm.test.tsx` (7 tests, fireEvent + a11y 어서션)
...
- `backend/src/config/env.validation.ts` — Joi 스키마. POSTGRES*\* / JWT*_ / BCRYPT*ROUNDS / COOKIE*_ / FRONTEND_ORIGIN 검증. custom으로 두 JWT 시크릿 동일 거부
...
- 루트 `.env` — POSTGRES*USER/PASSWORD/DB(기존) + POSTGRES_HOST/PORT(NestJS용) + JWT*\_/BCRYPT\_\_/COOKIE\_\*/FRONTEND_ORIGIN/NODE_ENV 통합. JWT 시크릿 `openssl rand -base64 48`로 다른 값 2개. (.gitignore에 .env 포함 확인)
...
- **결정 완료**: COOKIE*DOMAIN 미지정(ADR 0003), bcrypt dummy hash 모듈 import 시 1회 Promise(ADR 0002), env 검증 라이브러리 Joi, env 단일 소스 = 루트 .env, 변수 이름 POSTGRES*\*로 통일
