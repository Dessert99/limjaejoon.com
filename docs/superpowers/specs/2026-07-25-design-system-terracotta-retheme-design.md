...
정체성은 **색이 아니라 분위기(장소)** 로 정의한다: **황혼의 프라하 거리** — 테라코타 기와 지붕, 크림 회벽, 청동 녹청, 가스등 빛. 컨셉이 얹는 것은 새 브랜드색이 아니라 *따뜻한 빛과 공간감·재질*이다. 배경은 깊이감을 맡고, 색은 토큰이 책임진다.

> **2026-07-26 정정:** 최초 서술은 "배경(실사 이미지)이 색을 책임진다"였다. 플랜1에서 팔레트를 테라코타로 전면 교체하면서 색 책임이 토큰으로 넘어왔으므로 그 전제는 더 이상 유효하지 않다. 배경의 남은 역할은 깊이감뿐이며, 실사 사진은 채택하지 않는다(아래 비목표 참고).
...
- **프라하 파랄랙스 배경** — 별도 하위 프로젝트(스펙 분리). 기술: 레이어 2.5D 파랄랙스(GSAP). 이 스펙은 *컴포넌트 언어*에 한정한다. **2026-07-26 방향 변경:** 실사 포토 레이어 → **코드로 그린 실루엣 레이어**(SVG/CSS). 이유는 ① 색 책임이 이미 토큰으로 넘어감 ② 사진은 다크/라이트 2모드를 못 따라옴 ③ 다층 고해상도 이미지가 LCP를 직격 ④ 스톡 사진은 프론트 포트폴리오에서 본인 실력을 증명하지 못하며 "템플릿" 인상을 준다.
...
1. **컨셉 = 황혼의 프라하**, 분위기는 빛·공간·재질로. 배경은 깊이감(후속), 컴포넌트는 토큰.
