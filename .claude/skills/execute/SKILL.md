...
4. `docs/plans/<plan>/questions.md` — 사용자 질문 누적 로그. **없으면 아래 형식으로 새로 생성**:
...
라운드 단계와 무관하게, 매 사용자 메시지에서 **개념성 질문**이 들어오면 답변 후 즉시 `questions.md`에 append한다.

**캡처 대상**:
- "이게 뭐야?" / "왜 이렇게 해야 해?" / "원리가 뭐야?" / "어떻게 동작해?" / "차이가 뭐야?" 같은 이해 지향 질문
- 사용 중인 라이브러리·패턴·개념에 대한 호기심성 질문

**캡처 비대상**:
- 단순 진행 지시 ("다음 단계로", "고쳐줘", "이거 적용해")
- 결과 확인 ("됐어?", "맞아?")
- 작업 범위 결정 질문 (PRD·ADR 영역)
...
## Q<번호>. <질문 핵심 한 줄>
- 발생: <라운드 N 또는 단계 메모>
- A: <답변 요약. 필요 시 코드 예시 2~5줄>
```

질문 번호는 questions.md 안에서 1부터 단조 증가.
...
- [ ] 미해결 결정/질문: 갱신
- [ ] 질문 로그: 이번 라운드에 들어온 개념성 질문이 questions.md에 반영됐는지 확인
...
출력: `reports/YYYY-MM-DD-<기능명>.md` (study-note 스킬의 일반 형식 규약을 따른다).

**study-note 호출 시, 다음 두 섹션을 의무로 포함하도록 지시한다** (execute가 호출하는 study-note에 한정된 추가 요구):

1. **"구현 흐름"** — PRD/ADR을 짚으면서 도메인별로 무엇을 어떻게 만들었는지 학습자도 이해할 수 있게 풀어쓴다.
   - 단순 결과 나열이 아니라 "왜 이 구조를 골랐는가 → 어떻게 동작하는가"를 코드 예시(2~5줄, 원리 이해에 필요한 부분만)와 함께 설명.
   - 축약 금지. 처음 보는 학습자가 한 번 읽고 따라갈 수 있는 정도로 풀어쓴다.
   - ADR Learnings(6단계에서 채운 내용)도 함께 녹인다.

2. **"학습한 개념·패턴"** — 세션 중 사용자가 던진 질문을 정리한다.
   - **1차 입력**: `docs/plans/<plan>/questions.md` (전체)
   - 보조 입력: 대화 transcript에서 추가로 찾은 개념성 Q&A
   - 형식: 짧은 정의 → 왜 쓰는가 → 코드 예시(2~5줄) 또는 Q/A 그대로 인용
   - 단순 코드 변경 로그가 아닌 "이 작업을 통해 새로 알게 된 것" 위주

study-note 호출 시 `questions.md` 경로와 plan 폴더 경로를 명시적으로 입력으로 전달한다.
