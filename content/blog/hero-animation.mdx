---
title: 'Hero 등장 애니메이션 구현 방법'
date: '2026-03-31'
description: '프로젝트에 사용된 Vanilla Extract 애니메이션'
tags: ['Vanilla Extract', 'Project']
---
...
`keyframes`는 Vanilla Extract에서 CSS `@keyframes`를 타입 안전하게 작성하는 함수다. 빌드 시 고유한 클래스명(예: `fadeUp_abc123`)으로 변환된다.
...
엘리먼트가 처음 렌더링될 때 보이지 않도록 한다. 애니메이션이 시작되기 전 잠깐이라도 텍스트가 보이는 것을 막기 위해 필요하다.
...
세 요소가 같은 애니메이션을 갖되, 시작 시점을 0.2s씩 늦춘다. 이 덕분에 동시에 나타나지 않고 위에서 아래로 순서대로 등장하는 것처럼 보인다.
