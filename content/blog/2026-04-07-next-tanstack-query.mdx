...
3. 서버 컴포넌트에서 `prefetchQuery`를 실행하고, `dehydrate`한 상태를 `HydrationBoundary`에 넘긴다.
    ...
    export default async function PostsPage() {
      ...
      return (
        // HydrationBoundary: dehydrate된 JSON을 HTML 안에 심어 클라이언트로 전송
        // dehydrate(queryClient): 서버의 캐시 상태를 JSON으로 직렬화
...
4. 클라이언트 컴포넌트에서는 동일한 `queryKey`로 `useQuery`를 호출하면, 이미 캐시에 데이터가 있어 즉시 반환된다.
    ...
    export default function Posts() {
      // queryKey는 서버의 key와 동일해야 캐시가 연결된다
...
1. 아래 상황을 예시로 보자.
    ...
    export default async function PostsPage() {
      ...
      return (
        <HydrationBoundary state={dehydrate(queryClient)}>
          <div>게시글 수: {posts.length}</div> {/* fetchQuery로 가져온 데이터를 서버에서 직접 렌더링한 경우 */}
