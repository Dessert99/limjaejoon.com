...
4. Data Cache는 단순한 메모리 캐시가 아니라, 파일 시스템 수준(프로그램이 실행되는 동안만 유지되는 메모리(RAM)와 달리, 파일 시스템은 서버의 디스크(SSD/HDD)에 데이터를 저장하는 곳)에서 지속되는 영구 저장소다.
...
3. 기본값은 `'force-cache'`로(Next.js 14까지 해당, Next.js 15부터는 `'no-store'`가 기본값), 캐시가 존재하면 그것을 반환하고 없으면 새로 요청 후 저장한다.
